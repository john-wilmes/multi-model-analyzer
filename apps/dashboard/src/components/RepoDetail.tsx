...
export default function RepoDetail() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100">{repo}</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={label}
            className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border dark:border-slate-700 p-4"
          >
      ...
      {atdi && (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border dark:border-slate-700 p-4">
          <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200 mb-3">ATDI Score</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3 text-xs pt-1">
                <span className="text-red-600 dark:text-red-400">{atdi.findingCounts.error} errors</span>
                <span className="text-yellow-600 dark:text-yellow-400">{atdi.findingCounts.warning} warnings</span>
                <span className="text-blue-600 dark:text-blue-400">{atdi.findingCounts.note} notes</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {debt && (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border dark:border-slate-700 p-4">
          <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200 mb-3">Technical Debt</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
            <div className="flex flex-col items-center justify-center py-4">
              <p className="text-3xl font-bold text-slate-800 dark:text-slate-100">{debt.totalHours}h</p>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Total remediation effort</p>
            </div>
            <DebtBreakdownChart
              ...
              {coupling.pairs
                ...
                .map((p, i) => (
                  <tr key={i} className="border-b dark:border-slate-700 last:border-0">
                    <td className="py-2 pr-3 font-mono text-xs text-slate-600 dark:text-slate-400 truncate max-w-[200px]" title={p.fileA}>
                      {shortenPath(p.fileA)}
                    </td>
                    <td className="py-2 pr-3 font-mono text-xs text-slate-600 dark:text-slate-400 truncate max-w-[200px]" title={p.fileB}>
                      {shortenPath(p.fileB)}
                    </td>
                    <td className="py-2 pr-3 text-right text-slate-700 dark:text-slate-300">{p.coChangeCount}</td>
                    <td className="py-2 text-right text-slate-700 dark:text-slate-300">{(p.confidence * 100).toFixed(0)}%</td>
                  </tr>
                ))}
            </tbody>
          </table>
