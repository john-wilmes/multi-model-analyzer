...
import { splitIdentifier, analyzeNaming, analyzeNamingWithMeta } from "./naming.js";
...
describe("analyzeNamingWithMeta", () => {
  function sym(name: string, kind: SymbolInfo["kind"] = "function", containerName?: string): SymbolInfo {
    return { name, kind, startLine: 1, endLine: 5, exported: true, containerName };
  }

  it("meta.heuristic equals 'analyzeNaming'", () => {
    const result = analyzeNamingWithMeta(new Map(), "test-repo");
    expect(result.meta.heuristic).toBe("analyzeNaming");
  });

  it("meta.itemCount matches data.methods.length", () => {
    const files = new Map<string, readonly SymbolInfo[]>([
      ["src/user.ts", [sym("getUserById")]],
    ]);
    const result = analyzeNamingWithMeta(files, "test-repo");
    expect(result.meta.itemCount).toBe(result.data.methods.length);
    expect(result.meta.itemCount).toBe(1);
  });

  it("confidenceStats undefined for empty map, populated when methods exist", () => {
    const emptyResult = analyzeNamingWithMeta(new Map(), "test-repo");
    expect(emptyResult.meta.confidenceStats).toBeUndefined();

    const files = new Map<string, readonly SymbolInfo[]>([
      ["src/user.ts", [sym("getUserById")]],
    ]);
    const filledResult = analyzeNamingWithMeta(files, "test-repo");
