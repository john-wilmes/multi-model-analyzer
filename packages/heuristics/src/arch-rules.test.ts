...
describe("globMatch", () => {
  ...
  it("is anchored — rejects prefix/suffix mismatches", () => {
    expect(globMatch("prefix/src/db/query.ts", "src/db/**")).toBe(false);
    expect(globMatch("src/db/query.ts.bak", "src/db/*.ts")).toBe(false);
    expect(globMatch("xxsrc/db/query.ts", "src/db/**")).toBe(false);
  });
});
